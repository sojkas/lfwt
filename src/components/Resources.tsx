...
import Settings, {DepotUnit, MapMarker, Nimbee, Shift, Transporter,} from "../models/settings";
...
const Resources: React.FC<{
  ...
  const addUnitHandler = () => {
    setSelectedUnit((prevUnit) => {
      ...
      const settings = {...props.settings, depots : [...props.settings.depots, newDepot]};
    ...
    for (let depotUnit of props.settings.depots) {
      if (depotUnit.id === changedUnit.id) {
        return setSelectedUnit((prevUnit) => {
          ...
          newDepotArray.splice(newDepotArray.indexOf(depotUnit), 1,
              changedUnit
          );
  ...
  const setPositionHandler = (newPositon: google.maps.LatLng) => {
    setSelectedUnit((prevUnit) => {
      if (prevUnit?.marker) {
        ...
        return { ...prevUnit!, marker: newMarker };
      } else {
            ...
            <Grid container direction="column" spacing={2}>
              {props.settings.depots.map((depotUnit) => (
                  <Grid item rowSpacing={2}>
                    <DepotSetUnit
                        key={depotUnit.id}
                        depotUnit={depotUnit.id === selectedUnit?.id ? selectedUnit : depotUnit}
